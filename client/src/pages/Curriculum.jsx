...
export default function Curriculum_() {
  const { id } = useParams()
  ...
  useEffect(() => {
    api.getSession(id).then(d => { setContent(d.curriculum || ''); setLoading(false) }).catch(() => setLoading(false))
  }, [id])
        ...
        <div className="flex items-center justify-between mb-12">
          <div>
            <span className="inline-flex px-3 py-1 rounded-full text-[10px] uppercase tracking-[0.2em] font-medium mb-3"
              style={{ backgroundColor: 'var(--accent-soft)', color: 'var(--accent)' }}>Curriculum</span>
            <h1 className="text-3xl font-bold" style={{ fontFamily: 'var(--font-display)' }}>Learning Path</h1>
          </div>
          ...
          <div className="space-y-4">
            {[1,2,3].map(i => (
              <div key={i} className="p-[2px] rounded-[var(--radius-xl)]" style={{ background: 'var(--glass-border)' }}>
                <div className="h-24 rounded-[calc(var(--radius-xl)-2px)] bg-[var(--surface)] animate-pulse" />
              </div>
            ))}
          </div>
        ) : (
          <div className="p-[2px] rounded-[var(--radius-xl)]" style={{ background: 'var(--glass-border)' }}>
            <div className="rounded-[calc(var(--radius-xl)-2px)] p-8"
